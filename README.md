...
```bash
cd ApolloStation
git update-index --assume-unchanged apollo.int
```
Now git will ignore changes to the file apolloint.

### INSTALLATION

First-time installation should be fairly straightforward.  First, you'll need BYOND installed.  You can get it from [here](http://www.byond.com/).

This is a sourcecode-only release, so the next step is to compile the server files.  Open apollo.dme by double-clicking it, open the Build menu, and click compile.  This'll take a little while, and if everything's done right you'll get a message like this:

    saving apollo.dmb (DEBUG mode)
    
    apollo.dmb - 0 errors, 0 warnings
...
Finally, to start the server, run Dream Daemon and enter the path to your compiled apollo.dmb file.  Make sure to set the port to the one you  specified in the config.txt, and set the Security box to 'Trusted'.  Then press GO and the server should start up and be ready to join.
